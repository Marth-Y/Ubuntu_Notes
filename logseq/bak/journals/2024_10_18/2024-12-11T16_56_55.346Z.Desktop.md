- 有时候崩在一个地方，core dump文件一直指示同一个函数，但是log断在不同的位置，可能是多线程中另外一个线程崩溃导致的，如今天的可视化线程崩溃，每次log断在车道线，core dump文件一直指示的正确的崩溃函数，log让人误以为是断在车道线的，浪费了很多时间 #Work #Leanr #CPP/多线程 #core_dump
